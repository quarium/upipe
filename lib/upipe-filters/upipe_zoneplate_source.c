                                      ...
                                      uint32_t signature, va_list args)
{
    ...
    struct uref *flow_def_src = uref_dup(flow_def);
...
{
    UBASE_HANDLED_RETURN(upipe_zpsrc_control_src(upipe, command, args));
    UBASE_HANDLED_RETURN(upipe_zpsrc_control_bin_output(upipe, command, args));
