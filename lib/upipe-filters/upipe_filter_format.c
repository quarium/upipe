                            ...
                            struct uref *flow_def_dup)
{
    ...
    if (!ubase_ncmp(def, "pic.")) {
        ...
        struct upipe_ffmt_config config;
        upipe_ffmt_config_init(&config);
        if (unlikely(!ubase_check(upipe_ffmt_config_set(upipe, &config, flow_def, flow_def_dup)))) {
            abort();
        };
...
#define add_option(Fmt, ...) \
            ...
            if (in->surface_type == AV_QSV || out->surface_type == AV_QSV) {
                ...
                if (config.need_range)
                    add_option("out_range=%s",
                               out->fullrange ? "full" : "limited");
                if (color_matrix)
                ...
                add_option("tonemap=%d", config.need_tonemap ? 1 : 0);
                add_option("async_depth=0");
            } else if (out->hw) {
                if (config.need_deint && out->surface_type != AV_NI_QUADRA) {
                ...
                if (config.need_scale || config.need_format ||
                    config.need_range) {
                    if (out->surface_type == AV_NI_QUADRA) {
                ...
                if (config.need_tonemap && (in->surface_type == AV_VAAPI ||
                                            out->surface_type == AV_VAAPI)) {
                ...
                if (config.need_tonemap) {
                    ...
                    add_filter("zscale");
                    add_option("range=%s", out->fullrange ? "full" : "limited");
                    add_option("transfer=%s", color_transfer);
                    add_option("matrix=%s", color_matrix);
                    add_filter("format");
                    add_option("%s", pix_fmt_sw);
                } else if (config.need_scale || config.need_format ||
                           config.need_range) {
                    add_filter("scale");
                    add_option("interl=-1");
                    if (config.need_scale) {
