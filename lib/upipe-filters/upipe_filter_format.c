...
surface_type_from_str(const char *type)
{
    if (!type || !strcmp(type, ""))    return SW;
    if (!strcmp(type, "av.vaapi"))     return AV_VAAPI;
    if (!strcmp(type, "av.qsv"))       return AV_QSV;
    if (!strcmp(type, "av.ni_quadra")) return AV_NI_QUADRA;
    return UNKNOWN;
}
...
/** @hidden */
static int upipe_ffmt_fakedeint_probe(struct uprobe *uprobe,
                                      struct upipe *inner, int event,
                                      va_list args);
...
struct upipe_ffmt {
    ...
    /** proxy probe for fake deint */
    struct uprobe fakedeint_probe;
    ...
    /** allow flow format to be forwarded downstream for negotiation? */
    bool forward_flow_format;
    /** enforce pipeline even when input flow definition match wanted output */
    bool enforce;
...
UPIPE_HELPER_UPROBE(upipe_ffmt, urefcount_real, fakedeint_probe,
                    upipe_ffmt_fakedeint_probe)
...
UPIPE_HELPER_FLOW_DEF_CHECK(upipe_ffmt, flow_def_requested)
...
/** @internal @This catches events coming from the fake deint, and
 * attaches them to the bin pipe.
 *
 * @param uprobe pointer to the probe in upipe_ffmt_alloc
 * @param inner pointer to the inner pipe
...
static int upipe_ffmt_fakedeint_probe(struct uprobe *uprobe,
                                      ...
                                      va_list args)
{
    struct upipe_ffmt *upipe_ffmt = upipe_ffmt_from_fakedeint_probe(uprobe);
    struct upipe *upipe = upipe_ffmt_to_upipe(upipe_ffmt);
    ...
    switch (event) {
        ...
        case UPROBE_NEW_FLOW_DEF: {
            ...
            upipe_dbg(inner, "fake progressive");
            uref_pic_set_progressive(flow_def, true);
            uref_pic_delete_tff(flow_def);
            uref_dump_dbg(flow_def, inner->uprobe);
            return UBASE_ERR_NONE;
        }
    }
                                      ...
                                      uint32_t signature, va_list args)
{
    ...
    upipe_ffmt_init_fakedeint_probe(upipe);
    ...
    upipe_ffmt_init_flow_def_requested(upipe);
                                         ...
                                         struct uref *flow_def, const char *def)
{
    ...
    if (unlikely(flow_def_requested == NULL)) {
        upipe_ffmt_store_flow_def_requested(upipe, NULL);
    ...
    if (upipe_ffmt_check_flow_def_requested(upipe, flow_def_requested)) {
        ...
        upipe_ffmt_store_flow_def_provided(upipe, NULL);
        upipe_ffmt_store_flow_def_requested(upipe, flow_def_requested);
...
/** @internal @This loads values from a flow definition packet to a private
 * structure.
 *
 * @param flow private structure to load value into
 * @param flow_def flow definition packet to load value from
 * @return an error code
 */
...
/** @internal @This loads input and output values into a private structure.
 *
 * @param upipe description structure of the pipe
 * @param in input flow definition packet
 * @param out watned output flow definition packet
 * @return an error code
 */
    ...
    if (config->need_hw_derive) {
        if (config->in.surface_type == AV_VAAPI ||
            ...
            upipe_notice_va(upipe, "need hw surface mapping %s → %s",
                            surface_type_str(config->in.surface_type) ?: "sw",
                            surface_type_str(config->out.surface_type) ?: "sw");
        else
                                 ...
                                 const struct upipe_ffmt_config *config2)
{
    ...
    if (config1->need_interlace == config2->need_interlace &&
                                            ...
                                            struct upipe_ffmt_config *config)
{
    ...
    if (unlikely(!ffmt_mgr->deint_mgr)) {
        ...
        struct upipe *fake_deint = upipe_void_alloc(
            ...
            uprobe_pfx_alloc(
                uprobe_use(upipe_ffmt_to_fakedeint_probe(upipe_ffmt)),
...
#define add_option(Fmt, ...)                                                \
    pos += snprintf(buffer_get(buffer, pos, size), size_get(size, pos),     \
                                        ...
                                        struct uref *flow_def)
{
    ...
    if (upipe_ffmt_check_flow_def_provided(upipe, flow_def)) {
        // nothing changed
        uref_free(flow_def);
        return UBASE_ERR_NONE;
    }

    upipe_ffmt_store_flow_def_provided(upipe, flow_def);
...
static void upipe_ffmt_free(struct upipe *upipe)
{
    ...
    upipe_ffmt_clean_flow_def_requested(upipe);
    upipe_ffmt_clean_flow_def(upipe);
    upipe_ffmt_clean_fakedeint_probe(upipe);
